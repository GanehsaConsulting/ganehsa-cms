        ...
        <section className={`${isExpanded ? "w-64" : "w-[79px]"} ${noNavigation.includes(path) && "hidden"} sticky top-0 h-screen flex transition-all duration-300 z-[100]`}>
            <div className="relative w-full grow m-2 rounded-main bg-lightColor/15 dark:bg-darkColor/40 border border-lightColor/15 dark:border-darkColor/15 backdrop-blur-2xl flex flex-col">
                ...
                <div className="flex-shrink-0 bg-black/20 dark:bg-white/20 duration-300 rounded-secondary py-2 px-2 m-2 mb-0">
                    <div className={`${!isExpanded ? "opacity-100 flex items-center justify-center text-3xl" : "opacity-0 hidden"} relative duration-300 group`}>
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2 cursor-pointer">
                            ...
                            <div className={`${!isExpanded && "hidden"} text-lightColor text-sm font-bold`}>
                                CMS Ganesha
                            </div>
                        </div>
                        {isExpanded && (
                ...
                >
                    ...
                    {Object.entries(navigationItems).map(([key, section]) => (
                        <div key={key} className="mb-4">
                            <p className={`${!isExpanded && "w-full h-[1px] bg-neutral-500/30 rounded-full mb-3"} text-neutral-200 text-[10px] uppercase tracking-wide font-semibold px-3 pb-1`}>
                                <span className={`${!isExpanded && "hidden"}`}>
                                    {section.label}
                                </span>
                            </p>

                            <ul className={`${isExpanded ? "" : "items-center justify-center"} flex flex-col gap-2`}>
                                {section.contents.map((item, idx) => (
                                    <li key={idx} className={`${!isExpanded && item.subs?.length ? "dropdown dropdown-hover dropdown-right" : ""} relative`}>
                                            ...
                                            {!item.subs?.length ? (
                                                ...
                                                <>
                                                    ...
                                                    >
                                                        ...
                                                        {item.subs.length > 0 && isExpanded && (
                                                            <IoIosArrowDown
                                                                className={`transition-transform duration-200 ${openSubmenus[item.label] ? "rotate-180 text-lightColor" : "text-lightColor/60"}`}
                                                            />
                                                        )}
                                                    </button>
                ...
                <div className="flex-shrink-0 pb-2 px-2 space-y-2">
                    <div className={`${isExpanded ? "py-3 px-2 rounded-secondary " : "flex items-center justify-center rounded-full aspect-square"} bg-white/10 hover:bg-white/30 duration-300 mt-0`}>
                        <div className="flex items-center gap-3 cursor-pointer" onClick={toggleSidebar}>
                            ...
                            {isExpanded && (
                                <div className="flex items-center justify-between w-full">
                                    <div>
                                        <h2 className="text-white font-semibold text-sm">John Doe</h2>
