...
//   }, []);
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    //   formData.append("name", `Wallpaper ${currentUser?.name || "User"}`);
  ...
  const handleDeleteWallpaper = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        const defaultWallpaper = "https://i.pinimg.com/1200x/7a/f8/8c/7af88c637dd18de57bf938dec31c2248.jpg";
  ...
  return (
    <Wrapper className="flex flex-col gap-4 p-6">
      <h1 className="text-white text-xl font-semibold">Ganti Background</h1>

      <div className="bg-white/10 border border-white/20 p-4 rounded-xl ">
        <div className="mb-4">
          <p className="text-white text-sm mb-2">Upload wallpaper baru:</p>
          <input
            type="file"
            accept="image/*"
